...
class GateMatrixTests(unittest.TestCase):
    CASES = (
        (GateKind.CONFLICT_RESOLUTION, RunState.PROFILE_PENDING, "choose_value", True),
        (GateKind.CREDENTIAL, RunState.PROFILE_READY, "approve", True),
        (GateKind.SENSITIVE_DISCLOSURE, RunState.DRAFT_READY, "approve", True),
        (GateKind.DOMAIN_PIVOT, RunState.RESEARCH_READY, "approve", True),
        (GateKind.COVERAGE, RunState.AUDIT_RUNNING, "retry", False),
        (GateKind.EXCESSIVE_SIMILARITY, RunState.AUDIT_RUNNING, "retain_with_warning", True),
    )
    ...
    def test_every_gate_kind_resumes_only_its_recorded_state_and_operation(self):
        for kind, suspended_state, action, authorizes in self.CASES:
            with self.subTest(kind=kind), tempfile.TemporaryDirectory() as temporary:
                ...
                self.assertEqual(result.snapshot.state, suspended_state)
    ...
    def test_stop_is_terminal_for_every_gate_kind(self):
        for kind, suspended_state, _, _ in self.CASES:
            with self.subTest(kind=kind), tempfile.TemporaryDirectory() as temporary:
                connection, store, subject, scope, envelope = self._pending(temporary, kind, suspended_state)
                decision, result = store.decide_gate(
                    envelope.gate_id,
