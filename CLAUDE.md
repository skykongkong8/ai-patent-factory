...
4. Before putting private source text into model context, verify the transfer
   ...
   networked command: it and its free quota preflight egress only the bounded
   search parameters (keyword, country, result paging) and the
   `SERPAPI_API_KEY` value to `serpapi.com`, and only after the run and its
   state are validated locally. The key is never persisted and is
   canary-scrubbed from all artifacts. No command sends `documents/`,
   `workspace/`, or profile data to the network.
